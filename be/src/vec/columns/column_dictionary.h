...
//        LOG(INFO) << "[zeno] Dictionary::sort";
    ...
    public:
        ...
        inline void insert_value(StringValue* value) {
            // todo(zeno) log clean
            //            LOG(INFO) << "[zeno] Dictionary::insert_value value: " << value->to_string();
            dict_data.push_back_without_reserve(*value);
            inverted_index[*value] = inverted_index.size();
        }

        inline T get_index(const StringValue& value) const {
            // todo(zeno) log clean
            //            LOG(INFO) << "[zeno] Dictionary::get_index value: " << value.to_string();
        ...
        inline void get_indices(phmap::flat_hash_set<StringValue>& values) {
            // todo(zeno)
        }
